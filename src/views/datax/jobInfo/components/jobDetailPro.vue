...
<template>
  <div class="job_detail">
    ...
    <div class="header">
      ...
      <div class="header_action" @click="showJobSchedule">
      ...
      >
        ...
        >
          ...
          <el-form-item label="阻塞处理" prop="blockStrategy">
            <el-select
              v-model="scheduleForm.blockStrategy"
          ...
          <el-form-item label="路由策略" prop="routeStrategy">
            <el-select
              v-model="scheduleForm.routeStrategy"
              placeholder="请选择路由策略"
            >
              <el-option
          ...
          <el-form-item label="报警邮件" prop="alarmEmail">
            <el-input
              v-model="scheduleForm.alarmEmail"
              placeholder="输入报警邮件"
            />
          </el-form-item>
...
rkJggg=="
      ...
      <el-tab-pane label="查看JSON" name="json">
        <div class="json_content">
          <json-editor ref="jsonEditor" :value="jsons" cani-edit="nocursor" />
        </div>
      </el-tab-pane>
...
export default {
  ...
  computed: {
    ...
    childJob() {
      ...
      if (t) {
        ...
        const b = [];
        for (const aa of a) {
          const c = this.$store.state.taskAdmin.taskList.find(
            (ele) => ele.id === parseInt(aa)
          );
          if (c) {
            b.push(c);
          }
  ...
  methods: {
    ...
    initGoJs() {
      ...
      this.myDiagram.model = new go.GraphLinksModel(paramItem, paramLine);
    },
    /**
     * @description: 查看详情
     */
     ...
     */
    online(obj) {
      this.currentTask.triggerStatus =
        this.currentTask.triggerStatus === 0 ? 1 : 0;
