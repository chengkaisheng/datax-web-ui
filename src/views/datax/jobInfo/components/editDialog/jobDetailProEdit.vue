...
<template>
  <el-dialog :title="title" :visible="show" @close="$emit('close')">
          ...
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="数据源">
                ...
                >
                  <el-option
                    v-for="(item, index) in $store.state.taskAdmin
                      .dataSourceList"
                    :key="index"
            ...
            <el-col :span="12">
              ...
              <el-form-item v-if="hasSchema" label="Schema">
                <el-select
            ...
            <el-col :span="12">
              <el-form-item label="数据库表名" prop="tableName">
                <el-select v-model="writerTablesText" @change="wTbChange">
                  <el-option
            ...
            <el-col :span="12">
              <el-form-item
                ...
                prop="fileType"
              >
                <el-select v-model="writerFileType" placeholder="文件的类型">
            ...
            <el-col :span="12">
              <el-form-item
                ...
                prop="writeMode"
              >
                <el-select v-model="writeMode" placeholder="目标模式">
            ...
            <el-col :span="24">
              <el-form-item v-if="dataSource !== 'hive'" label="后置SQL">
                <el-input v-model="postSQL" placeholder="多个用;分隔" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="分区配置">
                <el-radio-group
                  v-model="writerPartition.partition"
                  @change="radioSelect"
                >
            ...
            <el-col :span="24">
              <el-form-item
                v-if="writerPartition.partition === 0"
                label="分区字段"
              >
                <el-select
                  v-model="writerPartition.partitionText"
                  placeholder="选择分区字段"
                  @change="columnSelect"
                >
                  <el-option
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button v-else type="primary" @click="handleShell"> 确定 </el-button>
    </span>
  </el-dialog>
</template>
  ...
  computed: {
    ...
    writerTablesText: {
      ...
      get() {
        if (this.taskParam.hasOwnProperty('writerTables')) {
          return this.taskParam.writerTables.length > 0
            ? this.taskParam.writerTables[0]
            : ''
        }
        return ''
      }
    ...
    readerTablesText() {
      if (this.taskParam.hasOwnProperty('readerTables')) {
        return this.taskParam.readerTables.length > 0
          ? this.taskParam.readerTables[0]
          : ''
      }
      return ''
    },
        ...
        if (this.currentTask.jobType !== 'SHELL') {
          if (this.currentTask.hasOwnProperty('jobParam')) {
            ...
            this.$store.commit(
              ...
              this.taskParam.readerDatasourceId
            )
          ...
          this.dataSource = this.$store.state.taskAdmin.dataSourceList.find(
            (ele) => {
  ...
  methods: {
    ...
    wDsChange() {
      this.$store.commit(
        ...
        this.taskParam.writerDatasourceId
      )
      this.dataSource = this.$store.state.taskAdmin.dataSourceList.find(
        (ele) => ele.id === this.taskParam.writerDatasourceId
      )?.datasource
    ...
    getColumns() {
      ...
      dsQueryGetColumns(obj).then((response) => {
        ...
        if (
          JSON.parse(this.currentTask.jobParam).writerTables[0] ===
            this.taskParam.writerTables[0] &&
          JSON.parse(this.currentTask.jobParam).writerDatasourceId ===
            this.taskParam.writerDatasourceId
        ) {
          this.$set(
            this.taskParam,
            'writerColumns',
            JSON.parse(this.currentTask.jobParam).writerColumns
          )
        } else {
          this.$set(this.taskParam, 'writerColumns', this.fromColumnList)
        }

        this.writerColumnsParam.checkAll =
          this.taskParam.writerColumns.length === this.fromColumnList.length
        this.writerColumnsParam.isIndeterminate =
          this.taskParam.writerColumns.length > 0 &&
          this.taskParam.writerColumns.length < this.fromColumnList.length
    ...
    wHandleCheckedChange(value) {
      ...
      this.writerColumnsParam.checkAll =
        checkedCount === this.fromColumnList.length
      this.writerColumnsParam.isIndeterminate =
    ...
    handleSubmit() {
      ...
      if (
        this.currentTask.glueType === 'BEAN' &&
        !isJSON(this.currentTask.jobJson)
      ) {
        this.$notify({
          title: 'Fail',
          message: 'json格式错误',
