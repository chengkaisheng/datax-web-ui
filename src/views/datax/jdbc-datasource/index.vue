  ...
  <div class="app-container">
    <div class="head-container">
      <el-card class="box-card">
        <div class="text item">
          <div class="left"><a class="backBtn" @click="$router.push('/datax/project/jobProject')"><i class="el-icon-back" /></a> 数据源管理</div>
      ...
      >
        ...
        <el-table-column label="数据源" width="100" align="center">
          <template slot-scope="scope">{{
            scope.row.datasource === "phoenix" ? "hbase" : scope.row.datasource
          }}</template>
        </el-table-column>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button size="small" type="goon" @click="testDataSource()">
          测试连接
        </el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog :visible.sync="dialogVisible" width="50%" top="51px">
      ...
      <div v-show="currentStep === 1" class="topSelect">
        ...
        >
          <el-button
            slot="append"
            style="backgroundColor: #3D5FFF;border-radius: 0px 4px 4px 0px;color: #FFFFFF;"
            size="small"
            @click="searchSQL"
          >搜索</el-button>
        </el-input>
      ...
      >
        <el-tab-pane label="所有类型" name="first">
          <ul>
            <li
              v-for="item in iconTXT"
              v-show="isShowSQL"
              :id="item.type + item.name"
              :key="item.name"
              @click="getInfo"
              @dblclick="dbClickNext"
            >
              <a>
    ...
    <el-dialog :visible.sync="dialogEditVisible" width="50%" top="51px">
      ...
      <div v-show="selectType === 'sqlserver'" class="sqlserForm">
        ...
        >
          <el-form-item label="数据源名称:">
            <el-input
              v-model="sqlserverEdit.datasourceName"
              placeholder="请输入数据源名称"
            />
          </el-form-item>
          <el-form-item label="备注:">
            <el-input
              v-model="sqlserverEdit.comments"
              placeholder="请输入备注"
            />
          </el-form-item>
          <el-form-item label="数据源分组:">
          ...
          <el-form-item label="认证:">
            <el-select
              v-model="sqlserverEdit.authentication"
              style="width: 100%;"
              placeholder="请选择"
            >
              <el-option label="master" value="master" />
      ...
      <div v-show="selectType === 'mongodb'" class="sqlserForm">
        ...
        >
          <el-form-item label="数据源名称:">
            <el-input
              v-model="MdbEdit.datasourceName"
              placeholder="请输入数据源名称"
            />
          </el-form-item>
          <el-form-item label="备注:">
      ...
      <!-- Greenplum PostqreSQL连接设置表单 -->
      <div
        v-show="selectType === 'greenplum' || selectType === 'postgresql'"
        class="set"
      >
        <p style="margin-top:0px;">基本信息</p>
        <div class="bgcForm">
    ...
    searchSQL() {
      for (let i = 0; i < this.typeArr.length; i++) {
        ...
        if (
          this.typeArr[i].name
            .toLowerCase()
            .indexOf(this.input3.toLowerCase()) === 0
        ) {
      ...
      if (this.selectType === 'mysql') {
        ...
        this.paramsData = {
          ...
          jdbcUrl:
            ...
            this.selectType +
            '://' +
            this.mysqlEdit.serverUrl +
            ':' +
            this.mysqlEdit.serverPort +
            '/' +
            this.mysqlEdit.database,
